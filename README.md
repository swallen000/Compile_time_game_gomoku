...
- [X] When the player select the same grid to place the stone, this move would be neglected. Also, the player’s turn remains.
...
Assume that we have a PRNG called `compile_time_random()` that can be called during compile-time. For how to implement such a function, please see https://mklimenko.github.io/english/2018/06/04/constexpr-random/
...
There is a magical variable `input` actually comes from the compile flag `-DInput` you will use for the `g++` command. (you will have a `constexpr User_Input input = User_Input::Input` elsewhere in your source code)
...
- We include .txt in compile time at every move, and this is the reason why the compile time is so slow. An insight here is that when deciding to build a game, managing the game in compile time would not give you a explicit boost on CPU time. 
