...
Assume that we have a PRNG called `compile_time_random()` that can be called during comiple-time. For how to implement such a function, please see https://mklimenko.github.io/english/2018/06/04/constexpr-random/
...
The a magical variable `input` actually comes from the compile flag `-DInput` you will use for the `g++` command. (you will have a `constexpr User_Input input = User_Input::Input` elsewhere in your source code)
